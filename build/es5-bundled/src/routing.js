...
"(?:\\:(\\w+)(?:\\(((?:\\\\.|[^\\\\()])+)\\))?|\\(((?:\\\\.|[^\\\\()])+)\\))([+*?])?"].join("|"),"g");/**
    ...
    */ /**
                                                                                                         ...
                                                                                                         * @return {!Array}
                                                                                                         */function parse(str,options){var tokens=[],key=0,index=0,path="",defaultDelimiter=options&&options.delimiter||DEFAULT_DELIMITER,delimiters=options&&options.delimiters||DEFAULT_DELIMITERS,pathEscaped=/* isRoot */ /* isRoot */!1,res;while(null!==(res=PATH_REGEXP.exec(str))){var m=res[0],escaped=res[1],offset=res.index;path+=str.slice(index,offset);index=offset+m.length;// Ignore already escaped sequences.
...
styleSet.forEach(function(v){return styles.unshift(v)})}else if(userStyles){styles.push(userStyles)}return styles}}]);return LitElement}(UpdatingElement);_exports.LitElement=LitElement;/**
   ...
   */LitElement.finalized=!0;/**
                                 ...
                                 * @nocollapse
                                 */LitElement.render=render$2;var litElement={LitElement:LitElement,defaultConverter:defaultConverter,notEqual:notEqual,UpdatingElement:UpdatingElement,customElement:customElement,property:property,query:query,queryAll:queryAll,eventOptions:eventOptions,html:html,svg:svg,TemplateResult:TemplateResult,SVGTemplateResult:SVGTemplateResult,supportsAdoptingStyleSheets:supportsAdoptingStyleSheets,CSSResult:CSSResult,unsafeCSS:unsafeCSS,css:css};_exports.$litElement=litElement;var router=new Router(document.getElementById("outlet"),{baseUrl:"/"});_exports.router=router;router.setRoutes([{path:"/",component:"home-view",action:function action(){new Promise(function(res,rej){return _require.default(["./components/home.js"],res,rej)}).then(function(bundle){return bundle&&bundle.$home||{}})}},{path:"/contact",component:"contact-view",action:function action(){new Promise(function(res,rej){return _require.default(["./components/contact.js"],res,rej)}).then(function(bundle){return bundle&&bundle.$contact||{}})}}]);var routing={router:router};_exports.$routing=routing});